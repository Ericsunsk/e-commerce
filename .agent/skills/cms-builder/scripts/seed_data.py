...
def seed_data(base_url, email, password):
    ...
    def ensure_record(collection, check_filter, data):
        ...
        existing = requests.get(list_url, headers=headers, verify=False).json().get('items', [])
        if existing:
    ...
    # 1. Categories
    cat_mens = ensure_record("categories", "slug='mens-wear'", {
        "name": "Men's Wear", "slug": "mens-wear", "sort_order": 10, "is_visible": True
    })
    cat_womens = ensure_record("categories", "slug='womens-wear'", {
        "name": "Women's Wear", "slug": "womens-wear", "sort_order": 20, "is_visible": True
    })

    # 2. Products
    if cat_mens:
        ensure_record("products", "slug='essential-tee'", {
    ...
    # 3. Global Settings
    ensure_record("global_settings", "site_name='ELEMENTHIC'", {
    ...
    # 4. Navigation
    ensure_record("navigation", "url='/shop'", {
        "location": "header", "label": "Shop", "url": "/shop", "order": 10, "is_visible": True
    })
    ensure_record("navigation", "url='/about'", {
        "location": "footer", "label": "About Us", "url": "/about", "order": 10, "is_visible": True
    })

    # 5. Pages
    page_home = ensure_record("pages", "slug='home'", {
    ...
    if page_home:
        ...
        existing = requests.get(list_url, headers=headers, verify=False).json().get('items', [])
        if not existing:
             requests.post(f"{base_url.rstrip('/')}/api/collections/ui_sections/records", json={
                 "page": page_home['id'],
                 "type": "hero",
    ...
    ensure_record("collection_images", "position='left'", {
    ...
    ensure_record("collection_images", "position='right'", {
        "title": "Shop Man",
        "position": "right",
