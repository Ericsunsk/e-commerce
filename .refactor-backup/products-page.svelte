...
<div class={ADMIN_PAGE.container}>
	...
	<div class={ADMIN_PAGE.header}>
		<div>
			...
			<p class={ADMIN_PAGE.subtitle}>监控商品生命周期、实时库存余量、多维属性与前台销售上架状态</p>
		</div>
	...
	<div class={ADMIN_CARDS.grid4}>
		...
		<div class={ADMIN_CARDS.kpi}>
			<div class="flex items-center justify-between">
				<span
					class="text-xs font-medium uppercase tracking-wider text-zinc-500 flex items-center gap-1.5"
				>
		...
		<div class={ADMIN_CARDS.kpi}>
			...
			<div class="mt-4">
				...
				<p class="text-[11px] text-zinc-400 mt-1">
					占比 {totalProducts > 0 ? Math.round((activeProducts / totalProducts) * 100) : 0}%
					正常接单中
				</p>
			</div>
		</div>
			...
			<div class="flex items-center gap-2 ml-auto">
				...
				<div class="relative">
					...
					{#if isFilterOpen}
						...
						>
							...
							<div>
								<div
									class="flex items-center justify-between text-[11px] font-semibold text-zinc-500 uppercase tracking-wider mb-2"
								>
									<span>所属分类</span>
									{#if selectedCategoryIds.length > 0}
										<button
									...
									<div class="flex flex-wrap gap-1.5 max-h-32 overflow-y-auto pr-1">
										{#each sortCategoriesByHierarchy(categories) as cat (cat.id)}
											...
											{@const count = rows.filter((r) =>
												r.categories?.some((c: { id: string }) => c.id === cat.id)
											).length}
											<button
												type="button"
												onclick={() => toggleCategoryFilter(cat.id)}
												class={isSelected
													? ADMIN_BUTTONS.filterPillActive
													: ADMIN_BUTTONS.filterPillInactive}
											>
												<span>{cat.name || cat.slug}</span>
												<span
													class={isSelected
														? ADMIN_BUTTONS.filterPillCountActive
														: ADMIN_BUTTONS.filterPillCountInactive}>({count})</span
												>
											</button>
										{/each}
									</div>
								...
								<div class="flex flex-wrap gap-1.5">
									...
									>
										...
										<span
											class={selectedStatuses.includes('active')
												? ADMIN_BUTTONS.filterPillCountActive
												: ADMIN_BUTTONS.filterPillCountInactive}>({activeProducts})</span
										>
									</button>
									...
									<button
										...
										class={selectedStatuses.includes('inactive')
											? ADMIN_BUTTONS.filterPillActive
											: ADMIN_BUTTONS.filterPillInactive}
									>
										<span>下架暂存</span>
										<span
											class={selectedStatuses.includes('inactive')
												? ADMIN_BUTTONS.filterPillCountActive
												: ADMIN_BUTTONS.filterPillCountInactive}>({inactiveProducts})</span
										>
									</button>
								</div>
							</div>
							...
							<div>
								...
								<div class="flex flex-wrap gap-1.5">
									<button
										...
										class={selectedStocks.includes('in_stock')
											? ADMIN_BUTTONS.filterPillActive
											: ADMIN_BUTTONS.filterPillInactive}
									>
										<span>库存充足</span>
										<span
											class={selectedStocks.includes('in_stock')
												? ADMIN_BUTTONS.filterPillCountActive
												: ADMIN_BUTTONS.filterPillCountInactive}>({inStockProducts})</span
										>
									</button>
									...
									<button
										...
										class={selectedStocks.includes('low_stock')
											? ADMIN_BUTTONS.filterPillActive
											: ADMIN_BUTTONS.filterPillInactive}
									>
										<span>库存紧张</span>
										<span
											class={selectedStocks.includes('low_stock')
												? ADMIN_BUTTONS.filterPillCountActive
												: ADMIN_BUTTONS.filterPillCountInactive}>({lowStockProducts})</span
										>
									</button>
									...
									<button
										...
										class={selectedStocks.includes('out_of_stock')
											? ADMIN_BUTTONS.filterPillActive
											: ADMIN_BUTTONS.filterPillInactive}
									>
										<span>已售罄</span>
										<span
											class={selectedStocks.includes('out_of_stock')
												? ADMIN_BUTTONS.filterPillCountActive
												: ADMIN_BUTTONS.filterPillCountInactive}>({outOfStockProducts})</span
										>
									</button>
								</div>
							</div>
				...
				<div class="relative">
					...
					{#if isSortOpen}
						...
						>
							<div
								class="px-2.5 py-1.5 text-[10px] font-bold text-zinc-400 uppercase tracking-wider"
							>
								排序方式
							</div>
							{#each sortOptions as opt (opt.id)}
								<button
									...
									class="w-full px-2.5 py-2 rounded-lg text-xs flex items-center justify-between transition-colors cursor-pointer {sortBy ===
									opt.id
						...
						{#each visibleRows as row (row.id)}
							<tr class={ADMIN_TABLE.row}>
									...
									<div class="flex items-center gap-3.5">
										...
										<div class="min-w-0 max-w-[280px]">
											<div class="flex items-center gap-1.5">
												...
												{#if row.isFeatured}
													<span
														class="inline-flex items-center text-[10px] font-semibold text-amber-700 bg-amber-50 px-1.5 py-0.2 rounded border border-amber-200/60 font-mono shrink-0"
													>
														精选
													</span>
												{/if}
								...
								<td class="{ADMIN_TABLE.cell} text-right">
									<span
										class="inline-block px-2.5 py-0.5 rounded-full text-[11px] font-mono font-bold border {row.totalStock ===
										0
											? ADMIN_BADGES.danger
											: row.totalStock <= 5
									...
									{#if row.categories && row.categories.length > 0}
										<div class="flex flex-wrap justify-center gap-1 max-w-[160px] mx-auto">
											{#each row.categories as cat (cat.id)}
												<span
													class="inline-block px-2 py-0.5 rounded-card bg-zinc-100 text-zinc-600 text-[10px] font-medium border border-zinc-200/60"
												>
													{cat.name}
												</span>
											{/each}
								...
								<td class="{ADMIN_TABLE.cell} text-right">
									<div class="inline-flex items-center gap-1.5">
										...
										<!-- Edit details -->
										<a href="/admin/products/{row.id}" class={ADMIN_BUTTONS.secondarySm}> 编辑 </a>
									</div>
								</td>
							</tr>
						{/each}
...
{#if inspectProduct}
	<div class={ADMIN_DRAWER.backdrop} role="dialog" aria-modal="true">
		<div class={ADMIN_DRAWER.panel}>
			...
			<div class={ADMIN_DRAWER.body}>
				...
				{#if !inspectProduct.variants || inspectProduct.variants.length === 0}
					<div
						class="p-8 bg-zinc-50 rounded-xl border border-zinc-200 text-center text-zinc-400 text-xs"
					>
						暂无独立规格记录（单品模式）
					</div>
				{:else}
					<div class="border border-zinc-200 rounded-xl overflow-hidden">
						<table class="w-full text-left text-xs border-collapse">
							<thead
								class="bg-zinc-50 border-b border-zinc-200 text-zinc-500 text-[10px] uppercase font-semibold"
							>
								<tr>
							...
							<tbody class="divide-y divide-zinc-100 font-mono">
								{#each inspectProduct.variants as v (v.sku || `${v.color}-${v.size}`)}
									<tr class="hover:bg-zinc-50/50">
										...
										<td
											class="py-2.5 px-3 text-right font-bold {v.stockQuantity === 0
												? 'text-rose-600'
												: v.stockQuantity <= 5
													? 'text-amber-600'
													: 'text-zinc-900'}"
			...
			<div class={ADMIN_DRAWER.footer}>
				...
				<a href="/admin/products/{inspectProduct.id}" class={ADMIN_BUTTONS.primary}>
					进入完整编辑
				</a>
			</div>
		</div>
...
{#if categoryModalOpen}
	...
	>
		...
		<div
			class="relative bg-white rounded-card shadow-2xl border border-zinc-200 w-full max-w-2xl max-h-[85vh] flex flex-col overflow-hidden z-10 animate-in fade-in zoom-in-95 duration-150"
		>
			<!-- Modal Header -->
			<div
				class="px-6 py-4 border-b border-zinc-200 bg-zinc-50 flex items-center justify-between shrink-0"
			>
				<div class={ADMIN_CARDS.sectionHeader}>
					...
					<div>
						<h2 class="text-sm font-bold uppercase tracking-wider text-zinc-900">分类管理</h2>
						<p class="text-[11px] text-zinc-500">管理目录结构、排序权重与前台分类可见性</p>
					</div>
				</div>

				<div class="flex items-center gap-2">
					{#if !showCategoryForm}
						<button type="button" onclick={openNewCategory} class={ADMIN_BUTTONS.primarySm}>
			...
			<div class="p-6 overflow-y-auto space-y-4 flex-1 text-xs text-zinc-700">
				...
				{#if showCategoryForm}
					<div
						class="p-4 rounded-card bg-zinc-50/90 border border-zinc-200 space-y-3 animate-in fade-in duration-150"
					>
						...
						<div class="flex flex-wrap items-center justify-between gap-2">
							<div class="flex items-center gap-2.5">
								...
								<div
									class="inline-flex p-0.5 bg-zinc-200/60 rounded-card border border-zinc-200/80"
								>
									{#each [{ id: 'gender', label: '一级类目' }, { id: 'primary', label: '二级类目' }, { id: 'subcategory', label: '三级类目' }] as opt (opt.id)}
										<button
											type="button"
											onclick={() => onCategoryTierChange(opt.id as CategoryTierOption)}
											class="px-2.5 py-1 rounded-card text-xs transition-all cursor-pointer {categoryForm.tier ===
											opt.id
						...
						<div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
							...
							<!-- Sort input -->
							<div
								class="flex items-center gap-1.5 bg-white border border-zinc-300 rounded-card px-2.5 py-1.5 shrink-0"
							>
				...
				<div class="border border-zinc-200 rounded-card overflow-hidden">
					{#if categoryLoading && categories.length === 0}
						<div class="py-12 text-center text-zinc-400">
							<span
								class="inline-block w-5 h-5 border-2 border-zinc-400 border-t-transparent rounded-full animate-spin mb-2"
							></span>
								...
								{#each sortCategoriesByHierarchy(categories) as cat (cat.id)}
									...
									<tr class="hover:bg-zinc-50/60 transition-colors">
										...
										<td class="py-2.5 px-3 text-center">
											<span
												class="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-medium border {tier.cls}"
											>
												{tier.label}
											</span>
										</td>
										<td class="py-2.5 px-3 text-center">
											<span
												class="inline-block px-2 py-0.5 rounded-card bg-zinc-100 text-zinc-700 font-mono text-[11px] font-medium"
											>
										...
										<td class="py-2.5 px-3 text-center">
											<button
												...
												class="inline-flex items-center gap-1 px-2 py-0.5 rounded-card border text-[11px] transition-colors cursor-pointer {cat.isActive !==
												false
			...
			<!-- Modal Footer -->
			<div
				class="px-6 py-3.5 border-t border-zinc-200 bg-zinc-50 flex items-center justify-between shrink-0"
			>
