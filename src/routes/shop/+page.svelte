<script lang="ts">
    ...
    let isNavLevelCategory = $derived(() => {
        ...
        return (
            currentCategory && data.navCategorySlugs.includes(currentCategory)
        );
    });

    // 是否显示子选项筛选栏
    let showCategoryFilter = $derived(
        $page.url.searchParams.has("gender") && !isNavLevelCategory(),
    );
    ...
    const unifiedTitle = $derived(
        activeSearch ? "SEARCH RESULTS" : data.page?.title || "COLLECTION",
    );
...
<div
    class="flex flex-col items-center w-full px-4 md:px-12 py-12 {data.sections
        ?.length
        ? ''
        : 'pt-32'}"
>
                    ...
                    >
                        ...
                        >
                            ...
                            >
                                FILTER
                                <span
                                    class="text-[16px] font-light leading-none"
                                    >+</span
                                >
                            </button>
                        </div>
        ...
        >
            ...
            {:else}
                {#each displayProducts as product (product.id)}
                    ...
                    >
                        <ProductCard {product} href="/shop/{product.id}" />
                    </div>
                {/each}
            {/if}
        </section>
...
>
    ...
    <div class="flex flex-col gap-10 pb-12">
        {#each Object.entries(filterGroups) as [groupName, options]}
            <div class="space-y-6">
                <h4
                        ...
                        >
                            <div class="relative flex items-center">
                                <input
                                    ...
                                    onchange={() =>
                                        toggleFilterOption(groupName, option)}
                                />
                            </div>
                            <span
