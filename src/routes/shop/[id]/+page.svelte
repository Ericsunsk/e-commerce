<script lang="ts">
    ...
    let derivedColors = $derived(
        product.variants?.length
            ? ([
                  ...new Set(
                      product.variants.map((v) => v.color).filter((c) => !!c),
                  ),
              ] as string[])
    ...
    let sortedImages = $derived.by(() => {
        ...
        if (variantImage) {
            ...
            const otherImages = defaultImages.filter(
                (img) => img !== variantImage,
            );
...
>
    <div class="lg:flex lg:gap-12 xl:gap-24">
        ...
        <div
            class="lg:w-[40%] xl:w-[35%] px-6 md:px-12 lg:px-0 lg:pr-12 mt-8 lg:mt-0"
        >
            <div class="sticky top-24 lg:max-w-[400px]">
                ...
                <div class="mb-8 space-y-4">
                    ...
                    <div
                        class="text-base font-sans text-primary dark:text-white"
                    >
                        {product.price}
                    </div>
                </div>
                            ...
                            {#each availableSizes as item}
                                {@const sizeLabel =
                                    (typeof item === "string"
                                        ...
                                        : item.size) || ""}
                                {@const isOutOfStock =
                                    typeof item === "object" && (item.stock || 0) <= 0}
                                <button
                                ...
                                >
                                    ...
                                    {#if isOutOfStock}
                                        <span
                                            class="block text-[8px] opacity-60"
                                            >SOLD OUT</span
                                        >
                                    {/if}
                                </button>
                ...
                <div class="flex gap-3 mb-10">
                    ...
                    >
                        <span
                            class="material-symbols-outlined text-[24px] font-light {wishlist.has(
                                product.id,
                            )
                                ? 'font-fill'
                                : ''}">favorite</span
                ...
                <div class="space-y-0">
                    ...
                    <div class="py-4">
                        <p
                            class="text-sm font-sans text-primary/80 dark:text-white/80 leading-relaxed"
                        >
                            {product.description}
                        </p>
                    </div>
                    ...
                    <details class="group py-4 cursor-pointer">
                        <summary
                            class="flex justify-between items-center list-none outline-none"
                        >
                            <span
                        ...
                        <div
                            class="pt-4 text-sm text-primary/70 dark:text-white/70 leading-relaxed"
                        >
    ...
    {#if relatedProducts && relatedProducts.length > 0}
        <section class="max-w-[1600px] mx-auto px-6 md:px-12 mt-32 mb-16">
            <h3
                class="font-display text-2xl mb-8 text-center uppercase tracking-widest"
            >
                You May Also Like
            </h3>
