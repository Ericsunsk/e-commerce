<script lang="ts">
	...
	import { COLORS, BUTTON_STYLES, TYPOGRAPHY, CONTENT_IMAGES } from '$lib/constants';
	...
	<div class="max-w-[1600px] mx-auto">
		{#if wishlist.items.length === 0}
			...
			>
				...
				>
					<RemoteImage
						src={data.settings?.emptyWishlistImage || CONTENT_IMAGES.WISHLIST_EMPTY}
		...
		{:else}
			...
			<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-4 gap-y-16">
				{#each wishlist.items as product (product.id)}
					...
					>
						<!-- Image Area -->
						<div class="relative aspect-[3/4] overflow-hidden bg-gray-100 dark:bg-zinc-800">
							<RemoteImage
								src={product.image || ''}
