			...
			{:else if step === 3}
				...
				>
					...
					>
						...
						{:else}
							Pay Now {formatMoney(
								pricing?.total ?? totalAmount ?? Math.max(0, cart.subtotal - discount)
							)}
						{/if}
					</Button>
					<button
			...
			<div class="space-y-4 mb-6">
				...
				{@render summaryRow(
					...
					step === 1
						...
						: (pricing?.shipping ?? (selectedShipping === 'express' ? 25 : 0)) > 0
							? formatMoney(pricing?.shipping ?? 25)
							: 'Free'
				)}
							...
							{formatMoney(
								pricing?.total ??
									Math.max(
										...
										cart.subtotal -
											discount +
											(step >= 2 ? (selectedShipping === 'express' ? 25 : 0) : 0)
									)
							)}
						</span>
					{/key}
