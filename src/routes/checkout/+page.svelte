<script lang="ts">
    ...
    async function initializePayment() {
        ...
        try {
                ...
                customerInfo: {
                    ...
                    userId:
                        auth.user && !auth.user.isAdmin
                            ? auth.user.id
                            : undefined, // Only link if regular user, not Admin
            ...
            if (!stripe) {
                throw new Error(
                    "Stripe not loaded. Check your PUBLIC_STRIPE_KEY.",
                );
            }
    ...
    // 共用样式
    const stepHeading =
        "text-xl font-display font-bold uppercase tracking-widest mb-6";
</script>

{#snippet summaryRow(label: string, value: string | number, highlight = false)}
    <div class="flex justify-between items-center text-xs {COLORS.text} font-bold uppercase tracking-wide {highlight ? 'text-red-500' : ''}">
        <span>{label}</span>
        <span>{value}</span>
    </div>
{/snippet}
...
<div class="min-h-screen {COLORS.bg} pt-32 pb-20">
    ...
    >
        ...
        <div class="lg:col-span-7">
            <!-- Progress Bar -->
            <div
                class="w-full h-[1px] bg-primary/10 dark:bg-white/10 mb-8 relative overflow-hidden"
            >
                <div
            ...
            <!-- Breadcrumbs -->
            <div
                class="flex items-center gap-2 text-xs uppercase tracking-widest mb-8 {COLORS.text}"
            >
                <span
                    class="{step >= 1
                        ? 'font-bold'
        ...
        >
            ...
            <div
                class="flex flex-col gap-6 mb-8 max-h-[40vh] overflow-y-auto scrollbar-hide"
            >
                {#each cart.items as item}
                    <div class="flex gap-4">
                        <div
                            class="w-16 aspect-[3/4] bg-primary/5 dark:bg-white/5 relative overflow-hidden"
                        >
                            <RemoteImage
                                src={item.image || ''}
                        ...
                        <div class="flex-1 flex flex-col justify-center">
                            ...
                            <p
                                class="text-[10px] {COLORS.text} uppercase tracking-wider"
                            >
            ...
            <div class="space-y-4 mb-6">
                ...
                <div class="flex gap-2">
                    ...
                    <input
                        ...
                        aria-describedby={couponMessage
                            ? "coupon-message"
                            : undefined}
                    />
                    <Button
                ...
                {@render summaryRow("Shipping", cart.total >= DEFAULTS.freeShippingThreshold ? "Free" : "Calculated at next step")}
                
                <!-- Tax placeholder -->
                {#if step === 3}
                    <div in:fade>
                        {@render summaryRow(`Tax (${taxAmount > 0 ? 'Included' : 'Estimated'})`, taxAmount > 0 ? formatMoney(taxAmount) : 'Calculated by Stripe')}
                    </div>
                {/if}
            </div>
