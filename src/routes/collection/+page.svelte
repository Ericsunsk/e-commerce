<script lang="ts">
    ...
    let leftRecord = $derived(
        data.collectionImages?.find((img: any) => img.position === "left"),
    );
    let rightRecord = $derived(
        data.collectionImages?.find((img: any) => img.position === "right"),
    );
...
<div class="relative w-full min-h-screen bg-background-light dark:bg-background-dark">
    ...
    <div class="relative w-full h-screen flex flex-col md:flex-row z-0">
        ...
        <a
            href={leftLink}
            class="flex-1 block bg-background-light dark:bg-primary overflow-hidden"
        >
            {#if heroImageLeft}
                <img
                    src={heroImageLeft}
    ...
    >
        <div class="mb-[calc(3rem+2.5vh)] text-center">
            <span
                class="text-[20px] font-medium tracking-[0.1em] uppercase text-primary dark:text-white"
                >Shop Now</span
            >
        </div>
        ...
        <div class="grid grid-cols-2 gap-4 md:grid-cols-4 md:gap-x-8 md:gap-y-12">
            {#each data.products as product}
                <a href="/shop/{product.id}" class="block">
                    <div
                        class="aspect-[3/4] w-full overflow-hidden bg-neutral-100 dark:bg-[#2a2a2a]"
                    >
                        <RemoteImage
                            src={product.image}
                            alt={product.title}
