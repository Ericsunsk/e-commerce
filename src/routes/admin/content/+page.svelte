...
<div class="space-y-6 max-w-3xl">
	<div>
		<h1 class="text-2xl font-display font-bold uppercase tracking-widest text-zinc-900">站点内容</h1>
	...
	<section class="bg-white border border-zinc-200 rounded-2xl shadow-xs overflow-hidden divide-y divide-zinc-100">
			...
			<label class="block p-5">
				...
				{#if field.type === 'boolean'}
					<button
						...
						class="relative inline-flex w-10 h-5 items-center rounded-full transition-colors {values[field.key] === true
							? 'bg-emerald-500'
							: 'bg-zinc-300'}"
					>
						<span
							class="inline-block w-3.5 h-3.5 rounded-full bg-white transition-transform {values[field.key] === true
								? 'translate-x-5'
								: 'translate-x-1'}"
						></span>
					</button>
		...
		{#if data.settings.iconUrl}
			<div class="p-5 flex items-center gap-4">
				<img src={data.settings.iconUrl} alt="站点图标" class="w-10 h-10 object-contain border border-zinc-200 rounded-lg" />
