...
import type { UserListsResponse, UserListsRecord } from '$lib/pocketbase-types';
...
async function getCartRecord(userId: string): Promise<UserListsResponse<CartItem[]> | null> {
    try {
        await initAdmin();
        const record = await pb.collection(Collections.UserLists).getFirstListItem<UserListsResponse<CartItem[]>>(
...
export const POST: RequestHandler = apiHandler(async ({ request, locals }) => {
    ...
    let cart = await getCartRecord(userId);
    let items: CartItem[] = cart?.items ? (cart.items as unknown as CartItem[]) : [];
