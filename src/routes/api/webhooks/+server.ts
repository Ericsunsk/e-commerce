...
import {
	deductProductStock,
	syncProductStatusFromStripe,
	createProductFromStripe
...
export const POST: RequestHandler = async ({ request }) => {
	...
	switch (event.type) {
		...
		case 'checkout.session.expired': {
			const _expiredSession = event.data.object as Stripe.Checkout.Session;
			break;
		}
...
async function handlePaymentIntentSucceeded(paymentIntent: Stripe.PaymentIntent) {
	try {
		...
		if (!order) {
			console.warn(
				`⚠️ No pending order found for PaymentIntent: ${paymentIntent.id}. This might be a standalone payment or order creation failed.`
			);
			return;
		}
 ...
 */
async function _handleProductCreated(stripeProduct: Stripe.Product) {
 ...
 */
async function _handleProductUpdated(stripeProduct: Stripe.Product) {
 ...
 */
async function _handleProductDeleted(stripeProduct: Stripe.Product) {
	const pbProductId = stripeProduct.metadata?.pb_product_id;
