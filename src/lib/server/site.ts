...
export async function getGlobalSettings(): Promise<GlobalSettings> {
    ...
    return withAdmin(async (pb) => {
        const [settingsResult, storyImg, aboutHeroImg, aboutSectionImg, emptyWishlistImg] = await Promise.all([
...
export async function getNavigation(location?: 'header' | 'footer' | 'mobile'): Promise<NavItem[]> {
    ...
    const fallbackNav = [
        { ...fallbackBase, id: '1', location: 'header', label: 'Collection', url: '/collection', order: 1, isVisible: true },
        { ...fallbackBase, id: '2', location: 'header', label: 'Mens', url: '/shop?gender=mens', order: 2, isVisible: true },
        { ...fallbackBase, id: '3', location: 'header', label: 'Womens', url: '/shop?gender=womens', order: 3, isVisible: true },
        { ...fallbackBase, id: '4', location: 'header', label: 'Accessories', url: '/shop?category=accessories', order: 4, isVisible: true },
    ] as NavItem[];
