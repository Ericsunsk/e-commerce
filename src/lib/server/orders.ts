...
import { OrdersStatusOptions, type OrdersResponse } from '$lib/pocketbase-types';
...
export async function getOrderBySessionId(sessionId: string): Promise<Order | null> {
    return withAdmin(async (pb) => {
        const record = await pb.collection(Collections.Orders).getFirstListItem(
            pb.filter('stripe_session_id = {:sessionId}', { sessionId })
        );
...
export async function updateOrderStatus(
    ...
    return withAdmin(async (pb) => {
        ...
        if (!allowedTransitions || !allowedTransitions.includes(status)) {
            console.warn(`⚠️ Invalid status transition from ${currentStatus} to ${status} for order ${orderId}`);
...
function mapRecordToOrder(record: any): Order {
    ...
    let items: OrderItem[] = record.items || [];
