import { pb, getFileUrl, initAdmin } from './pocketbase';
...
import { DEFAULTS, isValidSlug, COLLECTIONS, STOCK_STATUS } from '$lib/constants';
...
import { type ProductsResponse, type ProductVariantsResponse, ProductsStockStatusOptions } from '$lib/pocketbase-types';
 ...
 */
export async function deductProductStock(productId: string, quantity: number, variantId?: string): Promise<boolean> {
    const MAX_RETRIES = 3;
    let attempt = 0;
        ...
        try {
            ...
            if (variantId) {
                ...
                if (currentStock < quantity) {
                    console.warn(`⚠️ Insufficient stock for variant ${variantId}. Current: ${currentStock}, Requested: ${quantity}`);
                ...
                const checkVariant = await pb.collection(Collections.ProductVariants).getOne(variantId, { fields: 'updated,stock_quantity' });
                ...
                if (cvUpdated !== vUpdated || checkVariant.stock_quantity < quantity) {
                    console.warn(`Conflict detected for variant ${variantId}. Retrying... (${attempt + 1}/${MAX_RETRIES})`);
                    attempt++;
                ...
                console.log(`✅ Deducted ${quantity} from variant ${variantId} (Product: ${productId}). Remaining: ${newStock}`);
                return true;
                ...
                // ATOMICITY CHECK
                if (currentStock < quantity) {
                    console.warn(`⚠️ Insufficient stock for product ${productId}. Current: ${currentStock}, Requested: ${quantity}`);
                    return false;
                }
                ...
                // OCC Check
                const checkProduct = await pb.collection(Collections.Products).getOne(productId, { fields: 'updated,stock_quantity' });
                ...
                if (cpUpdated !== pUpdated || checkProduct.stock_quantity < quantity) {
                     console.warn(`Conflict detected for product ${productId}. Retrying... (${attempt + 1}/${MAX_RETRIES})`);
                     attempt++;
                ...
                console.log(`✅ Deducted ${quantity} from product ${product.title}. Remaining: ${newStock}`);
                return true;
            }
        } catch (e: any) {
            console.error(`❌ Failed to deduct stock for product ${productId} (Variant: ${variantId}) - Attempt ${attempt + 1}:`, e.message);
            attempt++;
 ...
 */
export async function syncProductStatusFromStripe(pbProductId: string, isActive: boolean): Promise<boolean> {
    try {
        await initAdmin();
 ...
 */
export async function createProductFromStripe(stripeProduct: Stripe.Product): Promise<ProductsResponse> {
    await initAdmin();

    const slug = stripeProduct.name.toLowerCase()
        .replace(/[^a-z0-9]+/g, '-')
        .replace(/(^-|-$)/g, '');
...
const priceCache = new Map<string, { data: { formatted: string; value: number }, expires: number }>();
...
async function fetchStripePrice(stripeId?: string): Promise<{ formatted: string; value: number }> {
    ...
    if (!secretKey || secretKey.startsWith('sk_test_placeholder') || stripeId.includes('TEST') || stripeId.includes('placeholder') || stripeId.includes('mock')) {
...
async function fetchStripePricesBulk(stripeIds: string[]): Promise<Map<string, { formatted: string; value: number }>> {
